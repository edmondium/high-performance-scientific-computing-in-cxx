...
void GPU_fill_random(float* x, int sz)
{
    ...
    curandGenerateUniform(cudagen, x, sz * sz);
}
void gpu_blas_mmul(const cublasHandle_t& handle, const float* A, const float* B, float* C, const int sz)
{
    int lda = sz, ldb = sz, ldc = sz;
    const float aa = 1;
    const float bb = 0.;
    const float* alpha = &aa;
    const float* beta = &bb;
    cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, sz, sz, sz, alpha, A, lda, B, ldb, beta, C, ldc);
}
...
int main(int argc, char* argv[])
{
    ...
    thrust::device_vector<float> d_A(size * size), d_B(size * size), d_C(size * size);
