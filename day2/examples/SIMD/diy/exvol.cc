...
auto exvolcalc(const vector_type<double>& R2, const vector_type<double>& S12) -> double
{
    ...
    for (size_t i = 0ul; i<vec_size ; i+=(unrollfactor*inc)) {
         for (size_t j = 0ul; j < unrollfactor; ++j) {
             r2b.unaligned_load(&R2[i+j*inc]);
             s12b.unaligned_load(&S12[i+j*inc]);
             totb = totb + Vexv(r2b, s12b);
         }
    }
    auto tot = totb.horizontal_add();
    ...
    for (size_t i=vec_size; i<R2.size(); ++i) tot += Vexv(R2[i], S12[i]);
    return tot;
#endif
}
