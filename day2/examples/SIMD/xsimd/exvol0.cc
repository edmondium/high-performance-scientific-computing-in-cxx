...
// THIS IS THE ONLY FUNCTION WHERE WE EXPLICITLY CHANGE THE LOOP
// TO USE SIMD INSTRUCTIONS
auto exvolcalc(const vector_type<double>& R2, const vector_type<double>& S12) -> double
{
    using b_type = xsimd::batch<double, current_arch>;
    ...
    for (size_t i = 0UL; i < vec_size; i += (unrollfactor * inc)) {
        for (size_t j = 0ul; j < unrollfactor; ++j) {
            r2b = b_type::load_unaligned(&R2[i + j * inc]);
            s12b = b_type::load_unaligned(&S12[i + j * inc]);
