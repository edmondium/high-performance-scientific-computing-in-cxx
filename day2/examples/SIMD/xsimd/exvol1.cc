...
struct exvolcalc_t {
    template <class Arch>
    auto operator()(Arch, std::span<const double> R2, std::span<const double> S12) const -> double
    {
        ...
        b_type totb { b_type::broadcast(0.) };
        for (size_t i = 0UL; i < vec_size; i += (unrollfactor * inc)) {
            for (size_t j = 0ul; j < unrollfactor; ++j) {
                b_type r2b = b_type::load_unaligned(&R2[i + j * inc]);
                b_type s12b = b_type::load_unaligned(&S12[i + j * inc]);
                totb = totb + Vexv(r2b, s12b);
            }
        }
        auto tot = reduce_add(totb);
        return std::inner_product(R2.begin() + vec_size, R2.end(), 
                                  S12.begin() + vec_size, 
                                  tot, 
                                  std::plus<double> {}, 
...
inline auto exvolcalc = xsimd::dispatch(exvolcalc_t {});
...
auto main(int argc, char* argv[]) -> int
{
    ...
    }
    {
        ...
        std::cout << "Time taken = " << tottime / nrep << " seconds\t :: " 
                  << nrep * 17.0 * R2.size() / tottime / 1.0e9 << " GFlops\n";
    }
}
