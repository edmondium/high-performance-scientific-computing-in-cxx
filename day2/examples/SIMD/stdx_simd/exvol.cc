...
#include <vector>
#include <span>
#if defined USE_SEQ_POLICY || defined USE_UNSEQ_POLICY 
#include <execution>
#endif
#ifdef USE_STDX_SIMD
...
#ifdef USE_ALIGNED_VECTORS
template <typename T>
using vector_type = std::vector<T, tbb::cache_aligned_allocator<T>>;
#if USE_STDX_SIMD
constexpr auto alignment_flag = stdx::vector_aligned;
#endif
#else
template <typename T>
using vector_type = std::vector<T>;
#if USE_STDX_SIMD
constexpr auto alignment_flag = stdx::element_aligned;
#endif
#endif
...
auto exvolcalc(std::span<const double> R2, std::span<const double> S12) -> double
{
#ifdef USE_STDX_SIMD
    ...
    for (size_t i = 0UL; i < vec_size; i += (unrollfactor * inc)) {
        for (size_t j = 0UL; j < unrollfactor; ++j) {
            r2b.copy_from(&R2[i + j * inc], alignment_flag);
            s12b.copy_from(&S12[i + j * inc], alignment_flag);
            totb = totb + Vexv(r2b, s12b);
        }
    }
    auto tot = reduce(totb);
...
#pragma omp simd reduction(+ : tot)
    for (size_t i = vec_size; i < R2.size(); ++i)
        tot += Vexv(R2[i], S12[i]);
    return tot;
#elif USE_UNSEQ_POLICY
    return std::transform_reduce(std::execution::unseq, 
		    R2.begin(), R2.end(), S12.begin(), 0., std::plus<double>{},
		    [](auto r2, auto s12) { return Vexv(r2, s12); });
#elif USE_SEQ_POLICY
